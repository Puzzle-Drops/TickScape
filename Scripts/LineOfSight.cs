...
/// 
/// COORDINATE SYSTEM:
/// Unity uses Y+ = North (opposite of SDK's Y+ = South)
/// This affects GetClosestPointTo calculations!
/// </summary>
public static class LineOfSight
{
    ...
    public static bool HasLineOfSight(int x1, int y1, int x2, int y2, int s = 1, int r = 1, bool isNPC = false)
    {
        ...
        // UNITY FIX: Y+ = North, so mob occupies from y1 to y1+s-1, not y1-s+1 to y1
        if (isNPC)
        {
            int tx = Mathf.Max(x1, Mathf.Min(x1 + s - 1, x2));
            int ty = Mathf.Max(y1, Mathf.Min(y1 + s - 1, y2)); // FIXED: Changed from y1 - s + 1
    ...
    /// 
    /// CRITICAL FIX: Unity uses Y+ = North, so we ADD yy, not subtract it!
    /// 
    /// For a 3x3 mob at gridPosition (16, 14):
    ///   SW corner: (16, 14) ← gridPosition
    ///   Occupies: X:[16-18], Y:[14-16] (in Unity)
    /// 
    /// All corner tiles:
    ///   (16,14), (17,14), (18,14)  ← Y+0 (SW row)
    ///   (16,15), (17,15), (18,15)  ← Y+1 (middle row)
    ///   (16,16), (17,16), (18,16)  ← Y+2 (NW row)
    /// </summary>
    private static Vector2Int GetClosestPointTo(Vector2Int sourcePos, Unit target)
    {
        List<Vector2Int> corners = new List<Vector2Int>();

        // Generate all corner tiles of the target unit
        // Unity: Y+ = North, so we ADD yy to go north
        for (int xx = 0; xx < target.size; xx++)
        {
            for (int yy = 0; yy < target.size; yy++)
            {
                corners.Add(new Vector2Int(
                    ...
                    target.gridPosition.y + yy  // FIXED: Was - yy (SDK), now + yy (Unity)
                ));
            }
        }

        // Find closest corner to source
