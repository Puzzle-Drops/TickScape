...
public class TileHighlight : MonoBehaviour
{
    ...
    void Start()
    {
        // Load colors from UISettings if available
        if (UISettings.Instance != null)
        {
            Debug.Log("[TileHighlight] Loading colors from UISettings");
        }
    ...
    /// <summary>
    /// Create the initial highlight GameObjects.
    /// </summary>
    private void CreateInitialHighlights()
    {
        // Get colors from UISettings (or use defaults if not available)
        Color hoverColor = UISettings.Instance != null ? UISettings.Instance.hoverColor : Color.white;
        Color playerColor = UISettings.Instance != null ? UISettings.Instance.playerTileColor : new Color(1f, 0.84f, 0f);
        Color destColor = UISettings.Instance != null ? UISettings.Instance.destinationColor : Color.white;

        // Create hover highlight (always 1x1) with color from settings
        hoverHighlight = CreateTileBorder("HoverHighlight", hoverMaterial, hoverColor, 1);

        // Create player tile highlight (always 1x1) with color from settings
        playerTileHighlight = CreateTileBorder("PlayerTileHighlight", playerTileMaterial, playerColor, 1);

        // Create destination highlight (always 1x1) with color from settings
        destinationHighlight = CreateTileBorder("DestinationHighlight", destinationMaterial, destColor, 1);
        ...
        Debug.Log("[TileHighlight] Initial highlights created");
    }

    /// <summary>
    /// CRITICAL: Rebuild all tile highlights with new colors from UISettings.
    /// Called when user changes colors in settings panel.
    /// </summary>
    public void RebuildAllHighlights()
    {
        if (UISettings.Instance == null)
        {
            Debug.LogError("[TileHighlight] Cannot rebuild - UISettings.Instance is null!");
            return;
        }

        Debug.Log("[TileHighlight] Rebuilding all highlights with new colors...");

        // Destroy old highlights
        if (hoverHighlight != null) Destroy(hoverHighlight);
        ...
        Debug.Log("[TileHighlight] All highlights rebuilt successfully!");
    }
    ...
    GameObject CreateTileBorder(string name, Material materialTemplate, Color color, int size)
    {
        ...
        if (materialTemplate != null)
        {
            ...
            // Apply the custom color from parameter
