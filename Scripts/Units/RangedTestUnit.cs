...
public class RangedTestUnit : Unit
{
    ...
    public override void PerformAttack()
    {
        ...
        // Calculate damage (simplified ranged calculation)
        int damage = Random.Range(0, maxHit + 1);

        Debug.Log($"[RANGED UNIT] Firing arrow for {damage} damage!");

        // Create visible projectile (like an arrow)
        ProjectileOptions options = new ProjectileOptions
        {
            hidden = false,  // Ranged projectiles are visible
            size = 0.3f,
            color = projectileColor,
            visualDelayTicks = 0,
            visualHitEarlyTicks = 0,
            // Arc motion for arrow
            motionInterpolator = new ArcProjectileMotion(1.5f)
        };

        // Create projectile with "range" attack style
        Projectile arrow = new Projectile(
            null, // No weapon object
            damage,
            this,
            aggro,
            "range",  // CRITICAL: Must be "range" for Protect from Missiles to work!
            options
        );
