...
public class InventoryPanel : BasePanel
{
    ...
    private void DrawItemSprite(Item item, float x, float y, float scale, bool isDimmed)
    {
        ...
        if (sprite != null)
        {
            ...
            Rect itemBounds = new Rect(x, y, ITEM_SIZE * scale, ITEM_SIZE * scale);
            Rect spriteRect = GetAspectFitRect(itemBounds, sprite);
    ...
    public override void OnCursorMoved(float relativeX, float relativeY)
    {
        ...
        // Update hover preview for tooltips and health bar preview
        UpdateHoverPreview(relativeX, relativeY);
    ...
    /// <summary>
    /// Update hover preview information for tooltips and bars.
    /// </summary>
    private void UpdateHoverPreview(float relativeX, float relativeY)
    {
        Item hoveredItem = GetItemAtPosition(relativeX, relativeY);

        // Only show hover for consumables and equipment
        if (hoveredItem != null && IsConsumableOrEquipment(hoveredItem))
        {
            Player player = FindPlayer();
            if (player != null)
            {
                // Store hover with current mouse position
                HoverPreview.SetHover(hoveredItem, player, Input.mousePosition);
            }
        }
        else
        {
    ...
    /// <summary>
    /// Check if item should show hover tooltip/preview.
    /// </summary>
    private bool IsConsumableOrEquipment(Item item)
    {
        return item is Food || item is Potion || item is Equipment;
    }
    ...
    public override void OnMouseUp()
    {
        ...
        // Clear hover preview when consuming items
        HoverPreview.Clear();
