...
public class InventoryPanel : BasePanel
{
    ...
    public override void DrawPanel(float x, float y, float scale)
    {
        ...
        // Draw highlight on hovered inventory slot (when dragging)
        if (draggedItem && clickedDownItem != null)
        {
            Item hoveredItem = GetItemAtPosition(cursorLocation.x, cursorLocation.y);
            if (hoveredItem != null)
            {
                // Get slot index
                int slotIndex = System.Array.IndexOf(inventoryCache, hoveredItem);
                if (slotIndex >= 0)
                {
                    int col = slotIndex % 4;
                    int row = slotIndex / 4;
                    DrawSlotHighlight(x, y, scale, col, row);
                }
            }
        }
    ...
    /// <summary>
    /// Draw slot highlight during drag (grey with transparency).
    /// </summary>
    private void DrawSlotHighlight(float panelX, float panelY, float scale, int col, int row)
    {
        float itemX = panelX + (20 + col * 43) * scale;
        float itemY = panelY + (17 + (row + 1) * 35) * scale;
        float slotSize = 32 * scale;

        Rect slotRect = new Rect(itemX, itemY, slotSize, slotSize);

        Color oldColor = GUI.color;
        GUI.color = new Color(0.5f, 0.5f, 0.5f, 0.3f); // Grey with transparency
    ...
    public override void OnMouseUp()
    {
        ...
        // Find drop target using last cursor position
        Item targetItem = GetItemAtPosition(cursorLocation.x, cursorLocation.y);

        if (targetItem != null && targetItem != clickedDownItem)
        {
            // Get positions
            int draggedPos = clickedDownItem.InventoryPosition(player);
            int targetPos = targetItem.InventoryPosition(player);

            if (draggedPos >= 0 && targetPos >= 0)
            {
                Debug.Log($"[InventoryPanel] Swapping: {clickedDownItem.itemName} (slot {draggedPos}) <-> {targetItem.itemName} (slot {targetPos})");

                // Update visual cache immediately for instant feedback
                inventoryCache[targetPos] = clickedDownItem;
                inventoryCache[draggedPos] = targetItem;
                ...
                if (InputManager.Instance != null)
                {
                    InputManager.Instance.QueueAction(() =>
                    {
                        player.SwapItemPositions(draggedPos, targetPos);
                    });
                }
                else
                {
        ...
        {
            string targetInfo = targetItem == null ? "null" : (targetItem == clickedDownItem ? "same item" : targetItem.itemName.ToString());
            Debug.Log($"[InventoryPanel] No swap: cursor at ({cursorLocation.x:F1}, {cursorLocation.y:F1}), target = {targetInfo}");
        }
