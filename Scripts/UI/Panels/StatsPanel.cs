...
public class StatsPanel : BasePanel
{
    ...
    private void DrawStat(Player player, string statName, float x, float y, float scale)
    {
        ...
        GUI.Label(new Rect(x - 15, y - 15, 50, 20), currentValue.ToString(), statStyleShadow);
        GUI.Label(new Rect(x - 16, y - 16, 50, 20), currentValue.ToString(), statStyle);

        // Base stat (bottom line) - shadow then main
        GUI.Label(new Rect(x + 5, y + 7, 50, 20), baseValue.ToString(), statStyleShadow);
        GUI.Label(new Rect(x + 4, y + 6, 50, 20), baseValue.ToString(), statStyle);
    }
