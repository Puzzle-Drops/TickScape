...
public class EquipmentPanel : BasePanel
{
    ...
    public override void DrawPanel(float x, float y, float scale)
    {
        ...
        GUI.Label(new Rect(x + 11 * scale, y + 251 * scale, 150, 20), weightText, weightStyleShadow);
        GUI.Label(new Rect(x + 10 * scale, y + 250 * scale, 150, 20), weightText, weightStyle);
    }
                                    ...
                                    float slotX, float slotY, float scale)
    {
        ...
        if (equipment != null)
        {
            ...
            if (sprite != null)
            {
                // Center the 32x32 sprite in the 36x36 slot (2px padding on each side)
                float spriteWidth = sprite.width * scale;
                float spriteHeight = sprite.height * scale;
                float xOffset = (36 * scale - spriteWidth) / 2f;
                float yOffset = (36 * scale - spriteHeight) / 2f;
