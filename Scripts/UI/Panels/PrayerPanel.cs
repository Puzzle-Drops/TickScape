...
public class PrayerPanel : BasePanel
{
    ...
    private const float PRAYER_ICON_SIZE = 30f; // Sprite size (leaves 3px padding per side)
    ...
    public override void Initialize()
    {
        ...
        // Load edit mode button texture
        editButtonTexture = TextureLoader.LoadTexture(
            "UI/Elements/prayer_edit_button",
    ...
    private void DrawPrayerAtGridPosition(Prayer prayer, float panelX, float panelY, float scale, Player player)
    {
        ...
        // Center the prayer icon within the cell (30x30 in 36x36 cell = 3px padding)
        float padding = (GRID_CELL_SIZE - PRAYER_ICON_SIZE) / 2f * scale;
        Rect iconRect = new Rect(
            cellRect.x + padding,
            cellRect.y + padding,
            PRAYER_ICON_SIZE * scale,
            PRAYER_ICON_SIZE * scale
        );

        // Get prayer sprite
        Texture2D sprite = GetPrayerSprite(prayer.type);
        ...
        if (prayer.isActive && !isDimmed)
        {
            Color oldColor = GUI.color;
            GUI.color = new Color(0.82f, 0.73f, 0.47f, 0.45f); // #D1BB7773
    ...
    private void DrawDraggedPrayer(float panelX, float panelY, float scale)
    {
        ...
        Texture2D sprite = GetPrayerSprite(draggedPrayer.type);
    ...
    private void DrawEditModeButton(float panelX, float panelY, float scale)
    {
        ...
        float buttonX = panelX + (204 * scale - buttonWidth) / 2f; // Center horizontally
        float buttonY = panelY + 250 * scale;

        Rect buttonRect = new Rect(buttonX, buttonY, buttonWidth, buttonHeight);

        // Draw button background
        Color oldColor = GUI.color;
        GUI.color = isEditMode ? new Color(0.2f, 0.8f, 0.2f, 0.5f) : new Color(0.5f, 0.5f, 0.5f, 0.5f);
        GUI.DrawTexture(buttonRect, Texture2D.whiteTexture);
        GUI.color = oldColor;

        // Draw button text
        GUIStyle buttonTextStyle = new GUIStyle();
        buttonTextStyle.fontSize = Mathf.RoundToInt(12 * scale);
    ...
    /// </summary>
    private Texture2D GetPrayerSprite(PrayerType type)
    {
        // Check cache
        if (prayerSpriteCache.ContainsKey(type))
        {
            return prayerSpriteCache[type];
        }
