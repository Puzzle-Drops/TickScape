...
public class PrayerPanel : BasePanel
{
    ...
    private void DrawPrayerAtGridPosition(Prayer prayer, float panelX, float panelY, float scale, Player player)
    {
        ...
        // Get prayer sprite (overhead version if LIT in panel, not if ACTIVE)
        // isLit = visual state (immediate), isActive = functional state (next tick)
        bool showOverhead = prayer.isLit && !(draggedPrayer == prayer && isDragging);
    ...
    /// <summary>
    /// Toggle prayer using the controller's proper toggle logic.
    /// SDK Reference: Prayer.toggle() in BasePrayer.ts
    /// </summary>
    private void TogglePrayer(Prayer prayer, Player player)
    {
        // Use the controller's toggle method which handles prayer flicking
        player.prayerController.TogglePrayer(prayer, player);
        Debug.Log($"[PrayerPanel] Toggled {prayer.GetName()} - Lit: {prayer.isLit}, NextState: {prayer.nextActiveState}");
    }
