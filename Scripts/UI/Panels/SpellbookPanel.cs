...
public class SpellbookPanel : BasePanel
{
    ...
    // Sprite cache (normal and selected versions)
    private Dictionary<SpellType, Texture2D> spellSpriteCache = new Dictionary<SpellType, Texture2D>();
    private Dictionary<SpellType, Texture2D> spellSpriteCacheSelected = new Dictionary<SpellType, Texture2D>();
    ...
    private void DrawSpellAtGridPosition(Spell spell, float panelX, float panelY, float scale)
    {
        ...
        // Get spell sprite (selected version if selected and not being dragged)
        bool showSelected = spell.isSelected && !(draggedSpell == spell && isDragging);
        Texture2D sprite = GetSpellSprite(spell.type, showSelected);
    ...
    private void DrawDraggedSpell(float panelX, float panelY, float scale)
    {
        ...
        Texture2D sprite = GetSpellSprite(draggedSpell.type, false);
    ...
    /// </summary>
    private Texture2D GetSpellSprite(SpellType type, bool selected = false)
    {
        // Choose the appropriate cache
        var cache = selected ? spellSpriteCacheSelected : spellSpriteCache;
        ...
        // Add suffix for selected version
        if (selected)
        {
            spriteName += "_selected";
        }
