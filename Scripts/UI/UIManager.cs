...
public class UIManager : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        if (player != null)
        {
            ...
            OrbsPanel.Draw(scale, player, panelsMinimized);
        }
    ...
    public bool IsMouseOverUI(Vector2 mousePos)
    {
        ...
        // Check orbs (compass, prayer, run)
        if (OrbsPanel.IsMouseOverOrbs(guiMousePos))
        {
            return true;
        }
        ...
        float hpBarWidth = 16f * UIScale.GetPanelScale();
        float panelHeight = 275f * UIScale.GetPanelScale();
        Rect hpRect = new Rect(hpPos.x, hpPos.y, hpBarWidth, panelHeight);
        if (hpRect.Contains(guiMousePos))
        {
            return true;
        }
        ...
        Rect prayerRect = new Rect(prayerPos.x, prayerPos.y, hpBarWidth, panelHeight);
        if (prayerRect.Contains(guiMousePos))
        {
            return true;
        }
    ...
    private void HandleMouseInput()
    {
        ...
        // Handle quick prayer selection first
        Player player = FindPlayer();

        // Handle context menu clicks
        if (Input.GetMouseButtonDown(0))
        {
            if (UIContextMenu.Instance.HandleClick(Input.mousePosition))
            {
                return;
            }
        }
        ...
        if (Input.GetMouseButtonDown(0))
        {
            if (player != null && OrbsPanel.HandleClick(mousePos, player))
            {
                return;
            }
        }

        // Handle orb right-clicks
        if (Input.GetMouseButtonDown(1))
        {
            if (player != null && OrbsPanel.HandleRightClick(Input.mousePosition, player))
            {
                return;
            }
        }
