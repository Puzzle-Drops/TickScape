...
public static class HealthPanel
{
    ...
    public static void Draw(float scale, Player player, bool panelsMinimized)
    {
        ...
        // Get hover preview changes
        int hpChange = HoverPreview.GetHPChange();
        int prayerChange = HoverPreview.GetPrayerChange();

        // Prayer bar always at same position
        Vector2 prayerPos = UILayout.GetPrayerBarPosition();
        DrawPrayerBar(prayerPos.x, prayerPos.y, barWidth, barHeight, scale, player, prayerChange);

        if (panelsMinimized)
        {
            // When minimized: HP bar moves to where panel was
            float hpX = prayerPos.x - barWidth - (4f * scale);
            DrawHealthBar(hpX, prayerPos.y, barWidth, barHeight, scale, player, hpChange);
        }
        else
        {
            // Normal layout: HP bar attached to panel left
            Vector2 hpPos = UILayout.GetHPBarPosition();
            DrawHealthBar(hpPos.x, hpPos.y, barWidth, barHeight, scale, player, hpChange);
        }
    }

    /// <summary>
    /// Draw the health bar (red) with optional preview.
    /// </summary>
    private static void DrawHealthBar(float x, float y, float barWidth, float barHeight, float scale, Player player, int hpPreviewChange = 0)
    {
        ...
        // Draw preview if hovering consumable
        if (hpPreviewChange != 0)
        {
            DrawHealthPreview(x, y, barWidth, barHeight, currentHP, maxHP, hpPreviewChange);
        }
    ...
    /// <summary>
    /// Draw semi-transparent preview of HP change.
    /// </summary>
    private static void DrawHealthPreview(float x, float y, float barWidth, float barHeight, int currentHP, int maxHP, int hpChange)
    {
        // Calculate preview HP (can show overheal slightly)
        int previewHP = currentHP + hpChange;
        float previewPercent = Mathf.Clamp01((float)previewHP / (float)maxHP); // Cap at 100% visual display

        // Calculate current and preview positions
        float currentPercent = Mathf.Clamp01((float)currentHP / (float)maxHP);

        // Preview section
        float previewY = y + (barHeight * (1f - previewPercent));
        float currentY = y + (barHeight * (1f - currentPercent));
        float previewHeight = Mathf.Abs(currentY - previewY);

        // Draw semi-transparent preview
        Color oldColor = GUI.color;
        GUI.color = new Color(0.8f, 0, 0, 0.5f); // Red, 50% opacity

        if (hpChange > 0)
        {
            // Healing - draw above current fill
            GUI.DrawTexture(new Rect(x, previewY, barWidth, previewHeight), Texture2D.whiteTexture);
        }
        else
        {
            // Damage - would draw below, but we don't show damage preview
            // (only healing preview makes sense for hover)
        }
    ...
    /// <summary>
    /// Draw the prayer bar (cyan) with optional preview.
    /// </summary>
    private static void DrawPrayerBar(float x, float y, float barWidth, float barHeight, float scale, Player player, int prayerPreviewChange = 0)
    {
        PlayerStats currentStats = player.currentStats as PlayerStats;
        PlayerStats baseStats = player.stats as PlayerStats;
        ...
        // Draw preview if hovering consumable
        if (prayerPreviewChange != 0)
        {
            DrawPrayerPreview(x, y, barWidth, barHeight, currentPrayer, maxPrayer, prayerPreviewChange);
        }
    ...
    /// <summary>
    /// Draw semi-transparent preview of prayer change.
    /// </summary>
    private static void DrawPrayerPreview(float x, float y, float barWidth, float barHeight, int currentPrayer, int maxPrayer, int prayerChange)
    {
        // Calculate preview prayer
        int previewPrayer = Mathf.Clamp(currentPrayer + prayerChange, 0, maxPrayer);
        float previewPercent = Mathf.Clamp01((float)previewPrayer / (float)maxPrayer);

        // Calculate current and preview positions
        float currentPercent = Mathf.Clamp01((float)currentPrayer / (float)maxPrayer);
        ...
        // Draw semi-transparent preview
        Color oldColor = GUI.color;
        GUI.color = new Color(0, 0.8f, 0.8f, 0.5f); // Cyan, 50% opacity

        if (prayerChange > 0)
        {
            // Restoring - draw above current fill
            GUI.DrawTexture(new Rect(x, previewY, barWidth, previewHeight), Texture2D.whiteTexture);
        }
