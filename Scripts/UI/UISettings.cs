...
public class UISettings : MonoBehaviour
{
    ...
    [Header("Quick Prayers")]
    [Tooltip("Prayers selected for quick activation")]
    public List<PrayerType> quickPrayerSelections = new List<PrayerType>();

    [Header("Compass")]
    [Tooltip("Is compass locked to north?")]
    public bool compassLocked = false;
    ...
    /// <summary>
    /// Reset tile highlight colors to defaults.
    /// </summary>
    public void ResetTileHighlightColorsToDefault()
    {
        hoverColor = Color.white;
        playerTileColor = new Color(1f, 0.84f, 0f); // Gold
        destinationColor = Color.white;
        npcTileColor = Color.red;
    }
    ...
    /// Format: "R,G,B,A" (0-1 float values)
    /// </summary>
    ...
    /// Format: "R,G,B,A"
    /// </summary>
    private Color StringToColor(string colorStr, Color defaultColor)
    {
        try
        {
        ...
        {
            Debug.LogError($"[UISettings] Failed to parse color '{colorStr}': {e.Message}");
            return defaultColor;
        }
    }
    ...
    public void SaveSettings()
    {
        ...
        PlayerPrefs.SetString("UI_PlayerTileColor", ColorToString(playerTileColor));
        PlayerPrefs.SetString("UI_DestinationColor", ColorToString(destinationColor));
        PlayerPrefs.SetString("UI_NPCTileColor", ColorToString(npcTileColor));

        // Save prayer and spell positions as serialized strings
        SavePrayerPositions();
        SaveSpellPositions();

        // Save quick prayer selections
        QuickPrayerData data = new QuickPrayerData { prayers = quickPrayerSelections };
        PlayerPrefs.SetString("QuickPrayers", JsonUtility.ToJson(data));

        // Save compass settings
        PlayerPrefs.SetInt("CompassLocked", compassLocked ? 1 : 0);
    ...
    public void LoadSettings()
    {
        ...
        hoverColor = StringToColor(
            PlayerPrefs.GetString("UI_HoverColor", ""),
            Color.white
        );
        playerTileColor = StringToColor(
            PlayerPrefs.GetString("UI_PlayerTileColor", ""),
            new Color(1f, 0.84f, 0f)
        );
        destinationColor = StringToColor(
            PlayerPrefs.GetString("UI_DestinationColor", ""),
            Color.white
        );
        npcTileColor = StringToColor(
            PlayerPrefs.GetString("UI_NPCTileColor", ""),
            Color.red
        );
        ...
        // Load quick prayer selections
        string quickPrayersJson = PlayerPrefs.GetString("QuickPrayers", "");
        if (string.IsNullOrEmpty(quickPrayersJson))
        {
            // Default selections
            quickPrayerSelections = new List<PrayerType>
            {
                PrayerType.PROTECT_FROM_MAGIC,
                PrayerType.AUGURY  // Boost Magic (was Augury)
            };
        }
        else
        {
            try
            {
                quickPrayerSelections = JsonUtility.FromJson<QuickPrayerData>(quickPrayersJson).prayers;
            }
            catch
            {
                quickPrayerSelections = new List<PrayerType>
        ...
        compassLocked = PlayerPrefs.GetInt("CompassLocked", 0) == 1;
...
[System.Serializable]
public class QuickPrayerData
{
    public List<PrayerType> prayers;
}