...
public class Mob : Unit
{
    ...
    /// 
    /// NOTE: GridToWorld returns the CENTER of a tile, not its SW corner.
    /// Tile prefabs are centered at origin, so no offset is needed for gizmos.
    /// </summary>
    void OnDrawGizmos()
    {
        ...
        foreach (var tile in xTiles)
        {
            ...
            // GridToWorld returns tile CENTER, so draw cube directly at that position
            Gizmos.DrawCube(worldPos + new Vector3(0, 0.1f, 0), new Vector3(0.9f, 0.1f, 0.9f));
        }
        ...
        if (!both && xOff != 0)
        {
            ...
            foreach (var tile in xOnlyTiles)
            {
                ...
                Vector3 worldPos = GridManager.Instance.GridToWorld(tile);
                // Slightly higher Y to layer on top of diagonal visualization
                Gizmos.DrawCube(worldPos + new Vector3(0, 0.15f, 0), new Vector3(0.8f, 0.1f, 0.8f));
            }
        }
        ...
        if (!both && !xSpace && yOff != 0)
        {
            ...
            foreach (var tile in yOnlyTiles)
            {
                ...
                Vector3 worldPos = GridManager.Instance.GridToWorld(tile);
                // Even higher Y to layer on top of other visualizations
                Gizmos.DrawCube(worldPos + new Vector3(0, 0.2f, 0), new Vector3(0.7f, 0.1f, 0.7f));
            }
        }
    }
