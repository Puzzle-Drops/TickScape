...
public class PlayerInput : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (mouseOverUI)
        {
            Debug.Log("[PlayerInput] Mouse is over UI - blocking input");
        }
        ...
        // Toggle running with R key (only if not binding keys)
        if (!mouseOverUI && !UISettings.Instance.isKeybinding && Input.GetKeyDown(KeyCode.R))
        {
