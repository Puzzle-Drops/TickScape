...
public class Player : Unit
{
    ...
    private const bool ENABLE_POSITION_DEBUG = false;
...
/// <summary>
    ...
    /// <summary>
    /// Temporary debug method to verify PlayerStats are working.
    /// Call this in Start() after SetStats() to verify.
    /// REMOVE after confirming it works!
    /// </summary>
    private void VerifyPlayerStats()
    {
        // Cast the inherited UnitStats field to PlayerStats
        if (stats is PlayerStats playerStats)
        {
            Debug.Log($"[PLAYER-STATS] ✅ Stats Type: PlayerStats");
            Debug.Log($"[PLAYER-STATS] Combat Stats: ATK={playerStats.attack} STR={playerStats.strength} DEF={playerStats.defence}");
            Debug.Log($"[PLAYER-STATS] Agility: {playerStats.agility}");
            Debug.Log($"[PLAYER-STATS] Run Energy: {playerStats.run}/10000 ({playerStats.run / 100}%)");
            Debug.Log($"[PLAYER-STATS] Special Attack: {playerStats.specialAttack}%");

            // Also verify currentStats
            if (currentStats is PlayerStats currentPlayerStats)
            {
                Debug.Log($"[PLAYER-STATS] ✅ CurrentStats also correctly typed as PlayerStats");
            }
        }
        else
        {
            Debug.LogError($"[PLAYER-STATS] ❌ ERROR! Stats type is: {stats.GetType().Name} (should be PlayerStats)");
        }
    }
