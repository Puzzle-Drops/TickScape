...
public class Player : Unit
{
    ...
    [Header("Effects")]
    [Tooltip("Active effects (poison, venom, stamina)")]
    ...
    public void EquipmentChanged()
    {
        ...
        // Handle special attack toggle when weapon changes
        if (equipment.weapon == null || !equipment.weapon.HasSpecialAttack())
        {
            useSpecialAttack = false;
        }
        // If weapon HAS special attack, keep useSpecialAttack as-is (preserve state)
    ...
    void Start()
    {
        ...
        if (inventory[0] == null)
        {
            ...
            testWeapon.attackRange = 1;
            testWeapon.attackSpeed = 4;
            testWeapon.weaponType = AttackStyleType.MELEE;

            // Enable special attack
            testWeapon.hasSpecialAttack = true;
            testWeapon.specialAttackDrain = 50; // Costs 50% energy

            inventory[16] = testWeapon;

            // Add test armor
            Equipment testHelm = new Equipment();
            testHelm.itemName = ItemName.ARMADYL_HELMET;
            testHelm.slot = EquipmentSlot.HELMET;
            inventory[17] = testHelm;

            // Add food (2 of each type)
            // Add food (2 of each type)
            Manta manta1 = new Manta();
            inventory[0] = manta1;

            Manta manta2 = new Manta();
            inventory[1] = manta2;
    ...
    public override void SetStats()
    {
        ...
        // CRITICAL: Clone as PlayerStats, not UnitStats
    ...
    public override void MovementStep()
    {
        ...
        // SDK Reference: Player.pretick() -> prayerController.tick()
        ...
        // Handle run energy drain and regeneration
        PlayerStats pStats = currentStats as PlayerStats;
        if (pStats != null)
        {
            // Determine if we're actually moving tiles this tick
            Vector2Int oldPos = gridPosition;

            // Temporarily check if we will move (preview the movement)
            bool willActuallyMove = false;
            if (!IsFrozen() && destinationLocation != null &&
                (gridPosition.x != destinationLocation.x || gridPosition.y != destinationLocation.y))
            {
                // We're trying to move - check if path exists
                int speed = running ? 2 : 1;
                Pathing.MoveResult moveResult = Pathing.Path(gridPosition, destinationLocation, speed, null);
                willActuallyMove = (moveResult.path != null && moveResult.path.Count > 0);
            }

            // Drain energy ONLY when running AND actually moving tiles
            if (running && willActuallyMove)
            {
                // Formula: ⌊60 + 67 * clamp[0,64](weight) / 64⌋ * (1 - agility / 300)
                float weight = GetWeight();
                float clampedWeight = Mathf.Clamp(weight, 0f, 64f);
                float baseReduction = Mathf.Floor(60 + (67 * clampedWeight) / 64);
                float agilityMultiplier = 1 - pStats.agility / 300f;
                int runReduction = Mathf.FloorToInt(baseReduction * agilityMultiplier);

                // Apply stamina effect if active
                if (effects.stamina > 0)
                {
                    runReduction = Mathf.FloorToInt(0.3f * runReduction);
                }

                pStats.run -= runReduction;
            }
            else
            {
                // Recover energy when NOT actually running
                // This includes: walking, standing still (even with run on), or blocked movement
                // Formula: ⌊agility / 10⌋ + 15
                int recovery = Mathf.FloorToInt(pStats.agility / 10f) + 15;
                pStats.run += recovery;
            }
            ...
            // Tick down stamina effect
            if (effects.stamina > 0)
            {
                effects.stamina--;
            }
        }
        // END OF RUN ENERGY BLOCK
    ...
    /// <summary>
    /// Activate prayers, set overhead, and play sounds.
    /// SDK Reference: Player.activatePrayers() in Player.ts lines 619-628
    /// </summary>
    private void ActivatePrayers()
    {
        ...
        // This will be used by OverheadPrayerRenderer component

        // Play prayer activation/deactivation sounds
        foreach (Prayer prayer in prayerController.prayers)
        {
            if (prayer.willPlayOffSound)
            {
                PlayPrayerSound(prayer, false);
                prayer.willPlayOffSound = false;
            }
            if (prayer.willPlayOnSound)
            {
    ...
    /// <summary>
    /// Play prayer sound from Resources folder.
    /// SDK Reference: Adapted from BasePrayer.playOnSound/playOffSound
    /// </summary>
    private void PlayPrayerSound(Prayer prayer, bool isActivation)
    {
        // Use Resources folder with naming convention
        // Example: "Audio/Prayers/protectmelee_on" or "protectmelee_off"
        string soundPath = $"Audio/Prayers/{prayer.GetSpriteName()}_{(isActivation ? "on" : "off")}";

        AudioClip clip = Resources.Load<AudioClip>(soundPath);
        if (clip != null)
        {
            AudioSource.PlayClipAtPoint(clip, transform.position, 0.35f);
        }
        // No error if sound doesn't exist - intentional for some prayers
    }
