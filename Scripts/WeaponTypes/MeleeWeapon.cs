...
public class MeleeWeapon : Weapon
{
    ...
    {
        // Check if target has an active protection prayer
        if (to.prayerController != null)
        {
            Prayer overhead = to.prayerController.GetOverhead();
            if (overhead != null && overhead.isActive)  // Check isActive, not just existence!
            {
                // For melee, all styles (slash/crush/stab) are blocked by Protect from Melee
                if (Weapon.IsMeleeAttackStyle(attackStyle) && overhead.GetFeature() == "melee")
                {
                    return true;
                }
            }
