...
/// 
/// ============================================================================
/// CRITICAL: COORDINATE SYSTEM DIFFERENCE FROM SDK
/// ============================================================================
/// SDK: Y+ = SOUTH (going south increases Y, going north decreases Y)
/// Unity: Y+ = NORTH (going north increases Y, going south decreases Y)
/// 
/// This affects the CollisionMath Y-axis calculations!
/// The SDK's collision formula has been adapted for Unity's coordinate system.
/// ============================================================================
/// 
...
public static class Collision
{
    ...
    /// 
    /// UNITY ADAPTATION:
    /// The SDK version assumes Y+ = South, but Unity uses Y+ = North.
    /// This means entity Y-ranges are calculated differently:
    /// 
    /// SDK (Y+ = South):
    ///   Entity at (x, y) size s occupies Y: [y-s+1, y]
    ///   (going north DECREASES Y)
    /// 
    /// Unity (Y+ = North):
    ///   Entity at (x, y) size s occupies Y: [y, y+s-1]
    ///   (going north INCREASES Y)
    /// 
    /// Example: 3x3 mob at (16, 14)
    ///   SDK would occupy: Y:[12, 14]
    ///   Unity occupies: Y:[14, 16]
    /// 
    /// Parameters:
    /// - (x, y, s): First rectangle's SW corner position and size
    /// - (x2, y2, s2): Second rectangle's SW corner position and size
    /// 
    ...
    {
        // Unity version with Y+ = North
        // Returns true if the two areas OVERLAP
        return !(x > x2 + s2 - 1 ||        // Entity 1 is east of Entity 2
                 x + s - 1 < x2 ||          // Entity 1 is west of Entity 2
                 y > y2 + s2 - 1 ||         // Entity 1 is north of Entity 2 (CHANGED from SDK)
                 y + s - 1 < y2);           // Entity 1 is south of Entity 2 (CHANGED from SDK)
    }
